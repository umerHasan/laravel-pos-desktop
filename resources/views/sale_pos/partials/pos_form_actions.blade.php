    ...
    <div
        ...
        <div
            ...
            <div class="!tw-w-full md:!tw-w-none !tw-flex md:!tw-hidden !tw-flex-row !tw-items-center !tw-gap-3">
                <!-- @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin')) 
                    <button type="button" 
                        ...
                        id="cardknox-pay" title="Pay with Cardknox">
                        <i class="fas fa-credit-card tw-text-[#D61B60]" aria-hidden="true"></i> Cardknox Pay
                    </button>
                @endif -->
            ...
            <div class="tw-flex tw-items-center tw-gap-4 tw-flex-row tw-overflow-x-auto">
				...
				@if (!Gate::check('disable_card') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <input type="hidden" id="cardknox_device_new" value="{{ $register_details->device_id }}">
                    <button type="button"
                        data-cardknox="true"
                        id="proceed_payment"
                        class="tw-font-bold tw-text-gray-700 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1  no-print @if (!empty($pos_settings['disable_suspend']))  @endif  @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif"
                        data-pay_method="card" title="@lang('lang_v1.tooltip_express_checkout_card')"><i class="fas fa-money-check-alt"
                            aria-hidden="true"></i> Cardknox Pay </button>
                @endif
				
				
                @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-hidden md:tw-flex md:tw-flex-row md:tw-items-center md:tw-justify-center md:tw-gap-1 tw-font-bold tb-text-white tw-text-white tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-[#001F3E] tw-rounded-md tw-p-2 tw-w-[8.5rem] @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_pay_checkout'] != 0) hide @endif"
                        id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')"><i class="fas fa-money-check-alt"
                            aria-hidden="true"></i> @lang('lang_v1.checkout_multi_pay') </button>
                @endif
                ...
                @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-font-bold tb-text-white tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-[rgb(40,183,123)] tw-p-2 tw-rounded-md tw-w-[8.5rem] tw-hidden md:tw-flex lg:tw-flex lg:tw-flex-row lg:tw-items-center lg:tw-justify-center lg:tw-gap-1 @if (!$is_mobile)  @endif no-print hide @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize"
			...
			<span class="tw-text-sm tw-text-gray-600"><span class="tw-font-bold">Station:</span> {{ $register_details->device->device_friendly_name ?? 'Not Assigned' }}</span>
...
<!-- <script>
    document.getElementById('cardknox-pay').addEventListener('click', function () {
        console.log('cardknox-pay clicked');
        const amount = parseFloat(document.getElementById('final_total_input').value) || 0.01;
        console.log('amount',amount);
        // fetch("{{ route('cardknox.initiate') }}", {
        //     method: "POST",
        //     headers: {
        ...
        // .then(res => res.json())
        // .then(data => {
        //     console.log("Cardknox response:", data);
        //     // Handle success/failure logic here (e.g., show modal, update UI)
        // })
        // .catch(err => {
