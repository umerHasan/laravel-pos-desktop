...
use App\Http\Controllers\CardknoxController;
...
Route::middleware(['setData'])->group(function () {
	...
	// Add webhook route here, outside of auth middleware
    Route::post('/cardknox/webhook', [App\Http\Controllers\CardknoxController::class, 'handleWebhook'])->name('cardknox.webhook');
});
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    Route::post('/cardknox/initiate', [CardknoxController::class, 'initiateTransaction'])->name('cardknox.initiate');
    Route::get('/cardknox/devices', [App\Http\Controllers\CardknoxController::class, 'getDevices'])->name('cardknox.devices');
    Route::get('/cardknox/devices-alt', [App\Http\Controllers\CardknoxController::class, 'getDevicesAlt'])->name('cardknox.devices-alt');
    Route::post('/cardknox/poll-session', [App\Http\Controllers\CardknoxController::class, 'pollSession'])->name('cardknox.poll-session');
    ...
    Route::post('/products/mass-notforselling', [ProductController::class, 'massNotforselling']);
    Route::post('/products/mass-forselling', [ProductController::class, 'massForselling']);
    ...
    Route::get('/change_return_amount', [SellPosController::class, 'change_return_amount']);
...
use App\Events\CardknoxPaymentCompleted;

Route::get('/test-cardknox-event', function () {
    $mockData = [
        'success' => true,
        'data' => [
    ...
    event(new CardknoxPaymentCompleted($mockData));
    return 'CardknoxPaymentCompleted event has been triggered!';
});

Route::get('/test-cardknox-webhook', function () {
    $mockWebhookData = [
        'xResponseResult' => 'Approved',
        'xAmount' => '0.01',
        'xRefNum' => '123456789',
        'xCommand' => 'CC:Sale',
        'xStatus' => 'Approved',
        'xToken' => 'test_token_123',
        'xInvoice' => 'INV-001',
        'xDescription' => 'Test webhook transaction'
    ];
    event(new \App\Events\CardknoxWebhookReceived($mockWebhookData));
    return 'CardknoxWebhookReceived event has been triggered!';
});
...
Route::get('/debug-pusher', function () {
    ...
    return view('debug-pusher', ['config' => $config]);
});

Route::post('/cardknox/challenge', [ChallengeController::class, 'challenge'])->withoutMiddleware(['web']);
