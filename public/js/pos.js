...
if (typeof APP !== 'undefined') {
    try {
        if (!APP.PUSHER_APP_KEY || !APP.PUSHER_APP_CLUSTER) {
            throw new Error('Pusher configuration is incomplete. Please check your environment variables.');
        }
        
        console.log('Initializing Pusher with configuration:', {
            key: APP.PUSHER_APP_KEY,
            cluster: APP.PUSHER_APP_CLUSTER
        });
        
        pusher = new Pusher(APP.PUSHER_APP_KEY, {
            cluster: APP.PUSHER_APP_CLUSTER,
            forceTLS: true
        });
        ...
        // pusher.logToConsole = true;
         ...
         // Subscribe to Cardknox webhooks channel
         try {
            const cardknoxChannel = pusher.subscribe('cardknox-webhooks');

            cardknoxChannel.bind('CardknoxWebhookReceived', function(data) {
                console.log('Received CardknoxWebhookReceived event:', data);
                // Get reference number from localStorage
                const storedSessionId = localStorage.getItem('currentxSessionId');
                console.log('Current stored reference number from localStorage:', storedSessionId);
                
                // Check if the webhook is for the current transaction
                if (storedSessionId && data.xSessionId === storedSessionId) {
                    // Handle the webhook data more gracefully
                    if (data.xGatewayStatus === 'Approved') {
                        toastr.success('Payment processed successfully');
                        
                        // Check which payment method was used and handle accordingly
                        if ($('#modal_payment').is(':visible')) {
                            console.log('called if');
                            // Second payment method - update UI elements
                            $('#proceed_payment_alt').text('Paid with Cardknox').prop('disabled', true);
                            $('#pos-save').prop('disabled', false);
                        }
                        else if ($('#proceed_payment').is(':visible')) {
                            console.log('called else if');
                            // First payment method - trigger cash process
                            $('.pos-express-finalize[data-pay_method="cash"]').trigger('click');
                        }
                        // Clear the reference number from localStorage after successful processing
                        localStorage.removeItem('currentxSessionId');
                    } else if (data.xResponseResult === 'TIMEOUT') {
                        toastr.error('Sorry, the payment session timed out. Please try again.');
                        // Clear the reference number from localStorage on timeout
                        localStorage.removeItem('currentxSessionId');
                    } else {
                        toastr.info('Payment status update: ' + (data.xResponseResult || 'Unknown status'));
                    }
                } else {
                    console.log('Received webhook for a different transaction or no active transaction');
                }
            });
            console.log('Successfully subscribed to Cardknox webhooks channel');
        } catch (error) {
            console.error('Error subscribing to Cardknox webhooks:', error);
            toastr.error('Error setting up payment notifications');
        }
    ...
    console.warn('Pusher is not enabled or APP configuration is missing');
}
...
$(document).ready(function() {
	...
	$('form#add_pos_sell_form').on('submit', function(e) {
        e.preventDefault(); // Prevent default form submission
		
		var amount = __read_number($('input#final_total_input')); // Get the total amount
		...
		$('#modal_payment').modal('hide'); // Hide payment modal
    });
    ...
    $('table#pos_table tbody').on('input keypress', 'input.pos_unit_price', function(e) {
        ...
        // If empty or only whitespace, set to 0.00 and position cursor at the end
        if (input.trim() === '') {
            $this.val('0.00');
        ...
        // Remove any invalid characters except digits
        let clean = input.replace(/[^\d]/g, '');
        
        // If completely empty after cleaning, set to 0.00
        if (clean === '') {
            $this.val('0.00');
            this.setSelectionRange(4, 4);
            return;
        ...
        const wholePart = clean.slice(0, -2);
        ...
        const formattedValue = `${cleanWholePart}.${decimalPart}`;
        
        // Update input value
        $this.val(formattedValue);
        
        // Always position cursor at the end (after decimal places)
        this.setSelectionRange(formattedValue.length, formattedValue.length);

		/*
		let value = $(this).val();
        let cursorPos = $(this)[0].selectionStart;
        let originalValue = value;

        // If empty or only whitespace, set to 0.00
        if (value.trim() === '') {
            $(this).val('0.00');
        ...
        if (value.includes('.')) {
            // If there's a dot, handle decimal formatting
            let dotIndex = value.indexOf('.');
            let beforeDot = value.substring(0, dotIndex);
            let afterDot = value.substring(dotIndex + 1);
            
            // If nothing before decimal point, add 0
            if (beforeDot === '') {
            beforeDot = '0';
            }
            ...
            // Preserve the original decimal part without forcing changes
            let formattedValue = beforeDot + '.' + afterDot;
            $(this).val(formattedValue);
            
            // Only adjust cursor if the value actually changed
            if (formattedValue !== originalValue) {
            $(this)[0].setSelectionRange(cursorPos, cursorPos);
            }
        } else if (!isNaN(value) && value !== '' && !value.endsWith('.')) {
            
            // Check if decimal point was removed from the original value
            if (originalValue.includes('.') && !value.includes('.')) {
               
            // Find where the decimal point was in the original value
            let originalDotIndex = originalValue.indexOf('.');
            
            // Add decimal point back at the same position
            let beforeDot = value.substring(0, originalDotIndex);
            let afterDot = value.substring(originalDotIndex);
            
            $(this).val(beforeDot + '.' + afterDot);
            $(this)[0].setSelectionRange(cursorPos, cursorPos);
            } else {
                ...
                if (value.length >= 2) {
                    ...
                    $(this).val(mainPart + '.' + decimalPart);
                    $(this)[0].setSelectionRange(cursorPos, cursorPos);
                }
                
            }
        } else if (value !== '' && !value.includes('.') && !isNaN(value.replace(/\./g, ''))) {
            // If decimal point was removed, add it back
            if (value !== '' && !value.includes('.') && !isNaN(value.replace(/\./g, ''))) {
                // Split value to insert decimal point
                if (value.length >= 2) {
                    let mainPart = value.slice(0, -2);
                    let decimalPart = value.slice(-2);
                    $(this).val(mainPart + '.' + decimalPart);
		...
		if(e.keyCode == 13) {
            e.preventDefault();
            $('.row_edit_product_price_model').modal('hide');
    ...
    $('button.pos-express-finalize').on('click', function() {
		// Check if product is present or not
		if ($('table#pos_table tbody').find('.product_row').length <= 0) {
			toastr.warning(LANG.no_products_added);
			return false;
		...
		console.log('Clicked pay method:', pay_method); // 🔍 Debug
		...
		if (total_payable > total_paying) {
			...
			__write_number(first_row, first_row_val + bal_due);
		...
		var payment_method_dropdown = $('#payment_rows_div')
			...
			.first();
		payment_method_dropdown.val(pay_method).change();
		
		// Show appropriate modal or submit
		if (pay_method == 'card') {
            if (!$(this).data('cardknox')) {
	...
	//on save cardknox details
    $('button#pos-save-cardknox').click(function() {
        $('input#card_number_1').val($('#card_number').val());
        $('input#card_holder_name_1').val($('#card_holder_name').val());
        $('input#card_transaction_number_1').val($('#card_transaction_number').val());
        $('select#card_type_1').val($('#card_type').val());
        $('input#card_month_1').val($('#card_month').val());
        $('input#card_year_1').val($('#card_year').val());
        $('input#card_security_1').val($('#card_security').val());

        $('div#cardknox_modal').modal('hide');
        pos_form_obj.submit();
    });
    ...
    pos_form_validator = pos_form_obj.validate({
	 ...
	 $('#modal_payment').on('keypress', 'input.payment-amount', function(e) {
        var paymentType = $('.payment_types_dropdown').val();
    
        if (e.keyCode === 13 && paymentType == 'cash') {
            e.preventDefault();
            calculate_balance_due();
            $('button#pos-save').trigger('click');
        }

    });
	
    // dev1 23062025
    $('.add-payment-custom').on('click', function() {
        var paymentType = $('.payment_types_dropdown').val();
        var amount = $(this).data('value');
        if(paymentType == 'cash') {
            $('input.payment-amount').val(amount).change();
                calculate_balance_due();
                // $('button#pos-save').trigger('click');
        }
        // add_payment_row(amount);
    });
	...
	// $(document).on('click', '#cardknox-pay', function () {
    ...
    //     $.ajax({
    //         url: '/cardknox/devices',
    //         method: 'GET',
    //         success: function(response) {
                
    //             // Remove existing modal if it exists
    //             $('#cardknox_device_modal').remove();
                
    //             // Append new modal to body
    //             $('body').append(response);
                
    //             // Initialize the modal
    //             var modal = $('#cardknox_device_modal');
    //             modal.modal({
    //                 backdrop: 'static',
    //                 keyboard: false
    //             });
    ...
    //             toastr.error('Error loading devices');
    //         }
    //     });
    // });

    // $(document).on('click', '#cardknox-pay-alt', function () {

    //     // Show device selection modal
    //     $.ajax({
    //         url: '/cardknox/devices-alt',
    //         method: 'GET',
    //         success: function(response) {
                
    //             // Remove existing modal if it exists
    //             $('#cardknox_device_modal_alt').remove();
                
    //             // Append new modal to body
    //             $('body').append(response);
                
    //             // Initialize the modal
    //             var modal = $('#cardknox_device_modal_alt');
    ...
    //     });
    // });
	function handleCardknoxInitiate(response) {
        console.log('response', response);
        if (response.data.xResult == 'S') {
            // Store reference number in localStorage
            localStorage.setItem('currentxSessionId', response.data.xSessionId);
            console.log('Stored Cardknox reference number:', response.data.xSessionId);
            if (response.success) {
                console.log("response", response);
                // $('#cardknox_device_modal').modal('hide');
    ...
    $(document).on('click', '#proceed_payment', function() {
        const deviceId = $('#cardknox_device_new').val();
        const amount = parseFloat(document.getElementById('final_total_input').value) || 0.01;
        ...
        $.ajax({
            ...
            success: function(response) {
                handleCardknoxInitiate(response);
            },
            error: function() {
                toastr.error('Error initiating payment');
            }
        });
    });
    // Handle proceed payment alt button click
    $(document).on('click', '#proceed_payment_alt', function() {
        const deviceId = $('#cardknox_device_alt').val();
        const amount = parseFloat(document.getElementById('final_total_input').value) || 0.01;
...
// on click sub category in category drawer
    ...
    $(document).on('change', '.payment_types_dropdown', function() {
        ...
        // Show the relevant payment details div
        $('.payment_details_div[data-type="' + paymentMethod + '"]').removeClass('hide');
        
        if (paymentMethod === 'custom_pay_1') {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.success) {
                    ...
                    console.log('result.html_modifier');
...
function pos_total_row() {
    ...
    $('span.unit_price_total').html(price_total);
    ...
    if (
        ...
        console.log('Updating serial no');
        update_serial_no();
    }
    save_pos_form_data();
}
...
// code by dev1 19062025
window.addEventListener("afterprint", (event) => {
    document.getElementById('search_product').focus();
    return_payment_modal();
//   console.log("After print jkhjkhkjhjkhjkhkjhkjhkjhkj");
//   alert('alert has been fixed');
});
// code by dev1 19062025


function return_payment_modal() {
    $.ajax({
        method: 'get',
        url: '/change_return_amount',
        dataType: 'json',
        
        success: (result) => {
            if (result.success && result.data) {
                // console.log('data',result.data);
                $('#return_payment_modal #total_quantity').text(__currency_trans_from_en(result.data.total_quantity,true) || '0');
                $('#return_payment_modal #total_payable_span').text(__currency_trans_from_en(result.data.total_payable_span,true) || '0');
                $('#return_payment_modal #total_paying').text(__currency_trans_from_en(result.data.total_pay,true) || '0');
                $('#return_payment_modal #change_return_span').text(__currency_trans_from_en(result.data.change_return_amount,true) || '0');
                $('#return_payment_modal').modal('show');
        ...
        error: function(xhr, status, error) {
            toastr.error('Failed to load return payment data');
        }
    });
...
$(document).on('keydown', function(e) {
    if (e.keyCode === 13) {
        $('#return_payment_modal').modal('hide');
    }
});