...
$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('keypress', 'input.pos_unit_price', function(e) {
        ...
        if(e.keyCode == 13) {
            ...
            // alert('Enter key pressed in unit price field. Please use tab key to move to next field.');
        }
    });
    ...
    $(document).on('click', '#proceed_payment', function() {
        const deviceId = $('#cardknox_device').val();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.data && response.data.xSessionId) {
                        let pollCount = 0;
                        const maxPolls = 5; // 5 minutes total
                        const pollInterval = 60000; // 1 minute in milliseconds
                        
                        const pollSession = () => {
                            if (pollCount >= maxPolls) {
                                toastr.error('Payment session timed out');
                                return;
                            }
                            
                            $.ajax({
                                url: '/cardknox/poll-session',
                                method: 'POST',
                                headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                                    'Content-Type': 'application/json'
                                ...
                                data: JSON.stringify({
                                    session_id: response.data.xSessionId,
                                    device_id: deviceId // include this line
                                }),
                                success: function(pollResponse) {
                                    if (pollResponse.success) {
                                        if (pollResponse.data.xSessionStatus === 'COMPLETED') {
                                            if (pollResponse.data.xGatewayStatus === 'Approved') {
                                                toastr.success('Payment approved successfully');
                                                // Trigger express checkout cash process
                                                $('.pos-express-finalize[data-pay_method="cash"]').trigger('click');
                                                return; // Stop polling
                                            } else {
                                                toastr.error('Payment was not approved');
                                                return; // Stop polling
                                            }
                                        } else if (pollResponse.data.xSessionStatus === 'TIMEOUT') {
                                            toastr.error('Sorry, the payment session timed out. Please try again.');
                                            return; // Stop polling
                                        } else {
                                            // Session is still in progress, continue polling
                                            pollCount++;
                                            if (pollCount < maxPolls) {
                                        ...
                                        toastr.error(pollResponse.message || 'Error checking payment status');
                                        return; // Stop polling
                                    }
                                },
                                error: function(xhr, status, error) {
                    ...
                    toastr.error(response.message || 'Error initiating payment');
                }
            },
            error: function(xhr, status, error) {
                const errorMessage = xhr.responseJSON?.message || 'Error processing payment';
                toastr.error(errorMessage);
            }
        });
    });

    // Handle proceed payment button click for alt modal
                                    ...
                                    if (pollResponse.success) {
                                        if (pollResponse.data.xSessionStatus === 'COMPLETED') {
                                            if (pollResponse.data.xGatewayStatus === 'Approved') {
                                                ...
                                                $('#cardknox-pay-alt').text('Paid with Cardknox').prop('disabled', true);
                                                // Enable the pos-save button
